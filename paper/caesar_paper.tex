...
Experiments on the real CICIDS2017 dataset (2.5M flows) demonstrate a
neutralization rate of $\mathbf{91.9\% \pm 3.3\%}$, robustness score of
$\mathbf{0.967 \pm 0.007}$, co-evolutionary gap of $\mathbf{+0.239}$
(Wilcoxon $p = 0.002$, Cohen's $d = 23.3$), and \mgae{} achieving
complete evasion of RF/DT classifiers (detection $98.5\% \to 0\%$).
The Autonomous Remediation Loop achieves 100\% autonomous healing success.
\end{abstract}
...
\begin{table}[htbp]
  \centering
  \caption{IDS comparison on real CICIDS2017 (200k test samples).}
  \label{tab:results}
  \begin{tabular}{@{}lcccc@{}}
    \toprule
    Model & F1 & DR & FPR & Robust. \\
    \midrule
    RF IDS      & 0.995 & 0.995 & 0.001 & --- \\
    DT IDS      & 0.993 & 0.998 & 0.002 & --- \\
    Threshold   & 0.383 & 0.754 & 0.445 & --- \\
    IDSGAN      & 0.996 & 0.995 & 0.001 & --- \\
    WGAN-IDS    & 0.996 & 0.996 & 0.001 & --- \\
    Deep RL     & 0.715 & 0.599 & 0.016 & --- \\
    \textbf{\caesar} & \textbf{0.977} & 0.421 & 0.150 & \textbf{0.967} \\
    \bottomrule
  \end{tabular}
\end{table}
...
attacker and a learning defender produces significantly more robust IDS
than any static baseline.
The \mgae{} engine's manifold-guided evasion opens a new direction for
adversarial robustness research in network security.
Future work will extend \caesar{} to real CICIDS2017 data, GNN-based
\tagraph{} reasoning, and federated multi-site deployment.
